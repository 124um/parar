...
pygame.display.set_caption("Scrolling Map with Coins, Dungeon, and Map Switching")
...
# Load the coin image
coin_image = pygame.image.load('coin.jpeg')
...
current_map_file = 'map1.txt'
map_data = read_map(current_map_file)
...
def switch_to_map(map_file, new_player_x, new_player_y):
    global map_data, map_width, map_height, player_x, player_y
...
while running:
    for event in pygame.event.get():
        ...
        if map_data[player_y][player_x] == 'D':
            ...
            dungeon_map = random.choice(['dange1.txt', 'dange2.txt'])
        ...
        if map_data[player_y][player_x] == 'E':
            ...
            switch_to_map('map1.txt', 8, 8)  # Return to the main map, starting at (8, 8)
    ...
    for row in range(map_height):
        for col in range(map_width):
            ...
            if tile == 'B':  # If tile is 'B', draw the green background and then the coin image
                # Draw green background
                pygame.draw.rect(screen, green, (tile_screen_x, tile_screen_y, tile_width, tile_height))
                # Draw the coin image on top of the green background
    ...
    pygame.draw.rect(screen, player_color, (player_screen_x, player_screen_y, tile_width, tile_height))
